...
class KrbApReq(KerberosV5Msg):
    ...
    def __init__(self, sequence):  # type: (Dict[int, ASN1Value]) -> None
        ap_options = get_sequence_value(sequence, 2, 'AP-REQ', 'ap-options', unpack_asn1_bit_string)
        self.ap_options = KerberosAPOptions(struct.unpack("<I", ap_options)[0])
...
class PAData:
    ...
    PARSE_MAP = [
        ...
        ('padata-value', 'value', ParseType.bytes),
    ]
...
class KdcReqBody:
    ...
    def unpack(value):  # type: (Union[bytes, ASN1Value]) -> KdcReqBody
        ...
        def unpack_addresses(value):
            return [unpack_addresses(h) for h in unpack_asn1_sequence(value)]
