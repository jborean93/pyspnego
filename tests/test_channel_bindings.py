...
TEST_B_DATA = b"\x02\x00\x00\x00\x04\x00\x00\x00\x01\x02\x03\x04" \
...
def test_channel_bindings_str():
    ...
    if sys.version_info[0] == 2:
        assert actual == r"GssChannelBindings initiator_addr(AddressType.inet|'\x01\x02\x03\x04') | " \
                         r"acceptor_addr(AddressType.unspecified|'\x05\x06\x07\x08') | " \
                         r"application_data('caf\xc3\xa9')"

    else:
        assert actual == r"GssChannelBindings initiator_addr(AddressType.inet|b'\x01\x02\x03\x04') | " \
...
def test_channel_bindings_repr():
    ...
    if sys.version_info[0] == 2:
        assert actual == r"spnego.channel_bindings.GssChannelBindings initiator_addrtype=<AddressType.inet: 2>|" \
