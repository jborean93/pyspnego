...
from spnego._ntlm_raw.messages import Authenticate, AvId, Challenge, Negotiate, SingleHost, TargetInfo
...
def test_parse_with_hex(capsys):
    entrypoint.main([
        '--token',
        base64.b16encode(get_data('ntlm_negotiate')).decode().lower()
    ])
...
def test_parse_with_file(capsys):
    entrypoint.main([
        '--file',
        os.path.join(os.path.dirname(__file__), 'data', 'ntlm_negotiate')
    ])
    actual_out = capsys.readouterr()
...
def test_parse_invalid_spnego_token(capsys):
    ...
    assert actual['Data']['responseToken']['Data'] == \
...
def test_ntlm_challenge(capsys):
    entrypoint.main([
        '--token',
        base64.b64encode(get_data('ntlm_challenge')).decode()
    ])
    actual_out = capsys.readouterr()
...
def test_ntlm_authenticate_ntlmv1(capsys):
    data = base64.b64encode(b"\x4E\x54\x4C\x4D\x53\x53\x50\x00"
                            b"\x03\x00\x00\x00"
                            b"\x18\x00"
                            b"\x18\x00"
                            b"\x6C\x00\x00\x00"
                            ...
                            b"\x51\x88\x22\xB1\xB3\xF3\x50\xC8\x95\x86\x82\xEC\xBB\x3E\x3C\xB7").decode()
    ...
    assert actual['Data']['Payload']['LmChallengeResponse']['LMProofStr'] == \
        '98DEF7B87F88AA5DAFE2DF779688A172DEF11C7D5CCDEF13'
    assert actual['Data']['Payload']['NtChallengeResponse']['ResponseType'] == 'NTLMv1'
    assert actual['Data']['Payload']['NtChallengeResponse']['NTProofStr'] == \
...
def test_ntlm_authenticate_lmv2(capsys):
    data = base64.b64encode(b"\x4E\x54\x4C\x4D\x53\x53\x50\x00"
                            ...
                            b"\x00\x00\x00\x00"
                            b"\x00\x00\x00\x00"
                            b"\xC5\xDA\xD2\x54\x4F\xC9\x79\x90\x94\xCE\x1C\xE9\x0B\xC9\xD0\x3E").decode()
...
def test_krb_tgs_req(capsys):
    ...
    assert actual['Data']['req-body']['sname'] == {
        'name-type': 'NT-SRV-HST (3)', 'name-string': ['HTTP', 'server2019.domain.local'],
    }
